...
test.describe("GET /api/[year]/circuits", async () => {
  test("should return circuits data for a valid year", async ({ request }) => {
    const response = await request.get(`/api/2024/circuits`)
  ...
  test("should respect limit and offset query parameters", async ({
    ...
    const response = await request.get(
      `/api/2024/circuits?limit=${limit}&offset=${offset}`
    )
    expect(response.ok()).toBeTruthy()
    expect(response.status()).toBe(200)
  ...
  test("should return 404 when no circuits are found", async ({ request }) => {
    const response = await request.get(`/api/inventedYear/circuits`)
