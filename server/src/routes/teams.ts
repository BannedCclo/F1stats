...
interface TeamSeasonClassification {
  championshipId: string
  season: number | null
  ...
  wins: number
  /**
   * Races entered that season. Always populated — for a lot of early/minor
   * entrants the constructors' championship itself was never tracked (no
   * position/points/wins), so this is what keeps their history non-empty.
   */
  racesEntered: number
}

interface TeamClassificationsApiResponse extends BaseApiResponse {
  teamId: string
  classifications: TeamSeasonClassification[]
}
...
// GET /teams/:teamId/classifications
router.get(
  "/teams/:teamId/classifications",
  async (req: Request, res: Response) => {
    ...
    try {
      ...
      const classificationRows = await db
        .select({
          championshipId: constructorsClassifications.championshipId,
          season: championships.year,
          position: constructorsClassifications.position,
          points: constructorsClassifications.points,
          wins: constructorsClassifications.wins,
        })
        .from(constructorsClassifications)
        .innerJoin(
          championships,
          eq(
            constructorsClassifications.championshipId,
            championships.championshipId
          )
        )
        .where(eq(constructorsClassifications.teamId, teamId))

      // A lot of early/minor entrants never had a tracked constructors'
      // championship position — fall back to how many races they actually
      // entered that season so their history isn't just empty.
      const raceCountRows = await db
        .select({
          championshipId: races.championshipId,
          season: championships.year,
          racesEntered: count(results.resultId),
        })
        .from(results)
        .innerJoin(races, eq(results.raceId, races.raceId))
        .innerJoin(championships, eq(races.championshipId, championships.championshipId))
        .where(eq(results.teamId, teamId))
        .groupBy(races.championshipId, championships.year)

      const bySeasonId = new Map<string, TeamSeasonClassification>()

      for (const row of classificationRows) {
        if (!row.championshipId) continue
        bySeasonId.set(row.championshipId, {
      ...
      for (const row of raceCountRows) {
        if (!row.championshipId) continue
        const existing = bySeasonId.get(row.championshipId)
        if (existing) {
          existing.racesEntered = row.racesEntered
        } else {
          bySeasonId.set(row.championshipId, {
            championshipId: row.championshipId,
            season: row.season,
      ...
      if (bySeasonId.size === 0) {
        return apiNotFound(
          res,
          fullUrl,
          "No classifications found for this team, try with other one."
        )
      }

      const classifications = [...bySeasonId.values()].sort(
        (a, b) => (a.season ?? 0) - (b.season ?? 0)
      )

      const response: TeamClassificationsApiResponse = {
        api: SITE_URL,
        url: fullUrl,
        teamId,
...
// GET /current/teams
// NOTE: must be registered before /:year/teams — Express matches routes in
